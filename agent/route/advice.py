...
def generate_route_advice(
    ...
    training_load = _find_training_load_summary(context)
    ...
    response = AnthropicMessagesClient().create_message(
        system=ROUTE_ADVICE_SYSTEM_PROMPT,
...
def _find_training_load_summary(context: AgentContext) -> dict[str, Any] | None:
    ...
    if not isinstance(result, dict):
        return None
    if result.get("schema_version") == "training_load_summary.v1":
        return result
    return None
